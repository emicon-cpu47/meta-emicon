...
#define CMP_OPCUA_SERVER_USER_AUTHENTICATION    {\
                                                    {(RTS_I32)CmpOPCUAServer_UserAuthentication_Disabled, CMPSECMAN_FLAGS_NONE, "DISABLED", "Deactivates the user authentication for the OPC UA server. Only Anonymus token will be available."},\
                                                    {(RTS_I32)CmpOPCUAServer_UserAuthentication_Enabled, CMPSECMAN_FLAGS_DEFAULT, "ENABLED", "Activates the user authenticaiton for the OPC UA Server. [Default]"},\
                                                    {(RTS_I32)CmpOPCUAServer_UserAuthentication_Enforced, CMPSECMAN_FLAGS_NONE, "ENFORCED", "Enforces that the users must login. No Anonymous token will be available."}\
                                                }
#define CMP_OPCUA_SERVER_SECSETTG_ID_USER_AUTHENTICATION    8
...
typedef enum
{
    CmpOPCUAServer_AllowUserPasswordOnPlaintext_Allowed,
	CmpOPCUAServer_AllowUserPasswordOnPlaintext_Forbidden
}CmpOPCUAServer_AllowUserPasswordOnPlaintext;
#define CMP_OPCUA_SERVER_ALLOW_USERPASSWORD_ON_PLAINTEXT    {\
                                                    {(RTS_I32)CmpOPCUAServer_AllowUserPasswordOnPlaintext_Allowed, CMPSECMAN_FLAGS_NONE, "ALLOWED", "Allows to transmit the password in a plaintext way."},\
                                                    {(RTS_I32)CmpOPCUAServer_AllowUserPasswordOnPlaintext_Forbidden, CMPSECMAN_FLAGS_DEFAULT, "FORBIDDEN", "Forbids to tramsit the password in a plaintext way."}\
                                                }
#define CMP_OPCUA_SERVER_SECSETTG_ID_ALLOW_USERPASSWORD_ON_PLAINTEXT    9
...
/**
 * <description>This function returns the user handle of the given session. This userhandle can be used with the CmpUserMgr to check 
 ...
 * <result>Handle of sessions user. RTS_INVALID_HANDLE if no user is available or an error occured.</result>
 */
