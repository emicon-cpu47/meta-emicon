...
#define EXPORT_EXTREF_STMT \
        { (RTS_VOID_FCTPTR)syssync, "syssync", 0xDC1119C9, 0x03050A00 },\
          { (RTS_VOID_FCTPTR)set_watchdog, "set_watchdog", 0x9CA67B6B, 0x03050A00 },\
          { (RTS_VOID_FCTPTR)set_inhibit, "set_inhibit", 0xAFE8BB40, 0x03050A00 },\
          { (RTS_VOID_FCTPTR)get_ptr_to_sock_arr, "get_ptr_to_sock_arr", 0x7F4ADBA2, 0x03050A00 },\
          { (RTS_VOID_FCTPTR)get_ptr_to_slot0, "get_ptr_to_slot0", 0xE6CB94A0, 0x03050A00 },\
          { (RTS_VOID_FCTPTR)get_ptr_to_dpm, "get_ptr_to_dpm", 0x7C8941E2, 0x03050A00 },\
...
class CCmpEmiconSys : public IEmiconSys 
{
    public:
        ...
        virtual void* QueryInterface(IBase *pIBase, ITFID iid, RTS_RESULT *pResult)
        {
            void *pItf;
            if (iid == ITFID_IBase)
                pItf = dynamic_cast<IBase *>((IEmiconSys *)this);            
            else if (iid == ITFID_IEmiconSys)
                pItf = dynamic_cast<IEmiconSys *>(this); 
            else
            {
                if (pResult != NULL)
